...
var KTDatatablesServerSide = (function () {
    ...
    var overlayblockUI = new KTBlockUI(overlaytarget, {
        message:
            '<div class="blockui-message"><span class="spinner-border text-primary"></span><span class="process_content">Processing Video</span></div>',
    });
    ...
    var validateForm = function () {
        $("#activity").on("change", function () {
            ...
            if (filter != "") {
                $("#commentlist").load(
                    "/employer/candidate/commentlist/" +
                        candidate_id +
                        "/" +
                        filter
                );
                ...
                $("#commentlist").load(
                    "/employer/candidate/commentlist/" + candidate_id
                );
            }
        });
            ...
            if (comment_val != "") {
                ...
                $.ajax({
                    ...
                    beforeSend: function () {
                        ...
                        $("#commentlist" + loopid).html(
                            '<div class="d-flex justify-content-center"><div class="spinner-border" style="width: 3rem; height: 3rem;" role="status"><span class="sr-only">Loading...</span></div></div>'
                        );
                    },
                    success: function (data) {
                        $("#cmnt_area").val("");
                        // overlayblockUIloop.release();
                        if (data.response == "1") {
                            $("#" + txtname).val("");
                            // $('#commentlist'+loopid).html(data.comments);
                            $("#commentlist" + loopid).load(
                                "/employer/qstncommentlist/" +
                                    question_id +
                                    "/" +
                                    candidate_id
                            );
    ...
    var videocontrols = function () {
        $(".video_player_box").each(function (index) {
            ...
            var finishtime = $("#upload_videotag" + index).attr(
                "data-finishedtime"
            );
            var loaddurtimetext = document.getElementById(
                "durtimetext" + index
            );
...
function vidSeek(seekslider, vid, finish_time, index) {
    ...
    seekslider.style.background = `linear-gradient(to right, white 0%, white ${
        ((value - min) / (max - min)) * 100
    }%, #b3b4b5 ${((value - min) / (max - min)) * 100}%, #b3b4b5 100%)`;

    seekslider.oninput = function () {
        this.style.background = `linear-gradient(to right, white 0%, white ${
...
function seektimeupdate(vid, seekslider, finish_time, index) {
    ...
    const max = seekslider.max;
    const value = seekslider.value;
    seekslider.style.background = `linear-gradient(to right, #0086FF 0%, #0086FF ${
        ((value - min) / (max - min)) * 100
    }%, #b3b4b5 ${((value - min) / (max - min)) * 100}%, #b3b4b5 100%)`;

    seekslider.oninput = function () {
        this.style.background = `linear-gradient(to right, #0086FF 0%, #0086FF ${
            ((this.value - this.min) / (this.max - this.min)) * 100
        }%, #b3b4b5 ${
            ((this.value - this.min) / (this.max - this.min)) * 100
        }%, #b3b4b5 100%)`;
