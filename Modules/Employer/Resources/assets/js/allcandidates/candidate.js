...
var KTDatatablesServerSide = function () {
    ...
    var dt;
    var filterPayment;
    const ratediv = document.querySelector('.ratingdiv');
    ...
    var initDatatable = function () {
        dt = $("#kt_table_candidates").DataTable({
            ...
            ajax: {
                url: "/employer/loadallcandidates",
                data: function ( d ) {
                    d.positionid = $('#position_id').val(),
                    d.candidatestatus = $('#candidate_status').val(),
                    d.star_rate =$('#star_rating').val(),
                    d.selectedpos = $('#searchpos').select2("val");
                }
            },
            columns: [
                { data: 'id' },
            ...
            },
            {
                ...
                render: function ( data, type, row ) {
                    ...
                    if(row.first_name!=null){
                        ...
                        return "<div class='d-flex flex-column justify-content-center'><a class='text-hover-primary text-dark' href='/employer/candidates/detail/" + row.encrypt_id  + "' >" + row.first_name +' '+ lname + "</a>\
                        <div class='fw-bold text-gray-400 fs-7'>"+row.position.name+"</div></div>" ;
                    }
                    return "<div class='d-flex flex-column justify-content-center'><span>N/A</span>\
                    <div class='fw-bold text-gray-400 fs-7'>"+row.position.name+"</div></div>" ;
                }
            },
            {
    ...
    var handleFilterDatatable = () => {
        ...
        $('#searchpos').on('change', function (e) {
           var searchva =  $('#searchpos').val();
           var rateval = $('#star_rating').val();
           if(searchva != ''){
              ratediv.classList.add('d-none');
              resetdiv.classList.remove('d-none');
           }else{
...
var handleResetForm = () => {
    ...
    resetButton.addEventListener('click', function () {
        ...
        $('#star_rating').val('');
        //$("#searchpos").empty();
        $('#searchpos').val(null).trigger('change');
        ...
        if (ids.length > 0) 
        {
            ...
            if (result.value) {
              ...
              $.ajax({
                ...
                url:"/employer/candidates/candidatemassdelete",
                method:"POST",
                data: { "id": ids },
                dataType:"json",
                success:function(data)
            ...
            swal.fire({
                ...
                if (result.value) {
                  ...
                  $.ajax({
                    ...
                    url:"/employer/candidates/sendmassinvite",
                    method:"POST",
                    data: { "id": ids },
                    dataType:"json",
                    success:function(data)
...
var toggleToolbars = function () {
    ...
    var overall_btn = '<button type="button" class="btn btn-danger" id="kt_datatable_delete_all" data-kt-candidate-table-select="delete_selected">Archive</button>';
    ...
    if(selected_status=='5'){
       overall_btn = '<button type="button" class="btn btn-primary" id="kt_datatable_restore_all" data-kt-candidate-table-select="restore_selected">Restore</button>';
        ...
        // Toggle toolbars
   if (checkedState) {
    toolbarSelected.innerHTML = overall_btn;
    ...
    $('.rating').raty({ 
        ...
        click: function(score) {
            ...
            $.ajax({
                ...
                type:"post",
                data: { 'candidate_id':candidateid,'rating':score},  
            });
        }
    });
  ...
  $(document).on("click", ".restore", function(e) {
    ...
    if (result.value) {
        ...
        $.ajax({
          ...
          url:"/employer/candidates/restore",
          type:"post",
          data: { 'cid': cid  },
          dataType:"json",
          success:function(data)
...
var loadstatusCount = function() { 


   $.ajax({

      url:"/employer/candidates/statuscount",
      type:"get",
    //   data: { 'pid': $('#pid').val()  },
      dataType:"json",
      success:function(data)
