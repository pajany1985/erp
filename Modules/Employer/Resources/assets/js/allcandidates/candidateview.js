    ...
    var rating = function(){
        ...
        $('.rating').raty({ 
            ...
            click: function(score) {
             ...
             $.ajax({
                url:"/employer/candidates/rating",
                type:"post",
                data: { 'candidate_id':cid,'rating':score},  
            });
         }
     });
     ...
     $(document).on("click", ".open_cmtmdl", function(e) {
        ...
        $('#cmnt_question_no').val(cmnt_qstinon);
        // $('#comment_modal').
        $('#comment_modal').modal({
            backdrop: 'static',
            keyboard: false
...
$('#restore').click(function() {
     ...
     $('#frmcandidate').attr('action', '/employer/candidates/restore');    
    $('#frmcandidate').submit();
...
var validateForm = function() {
    ...
    if(filter!='')
    {
        $('#commentlist').load("/employer/candidates/commentlist/" + candidate_id+"/"+filter);
    }
    else
    {
        $('#commentlist').load("/employer/candidates/commentlist/" + candidate_id);
    }
    ...
    $("#frmcomment").validate({
        ...
        $.ajax({
        url:"/employer/candidates/createactivity",
        type: 'post',
        data: {"candidate_id": candidate_id,"cmnt_area": cmnt_area },
        beforeSend: function(){
            overlayblockUI.block();
            $('#commentlist').html('<div class="d-flex justify-content-center"><div class="spinner-border" style="width: 3rem; height: 3rem;" role="status"><span class="sr-only">Loading...</span></div></div>');
        },
    ...
    $("#kt_candidates_comment_form").validate({
        ...
        submitHandler: function(form) {
            ...
            $.ajax({
                url:"/employer/candidates/questioncomment",
                type: 'post',
                data: { "cmnt_question_no": cmnt_question_no,"candidate_id": candidate_id,"cmnt_area": cmnt_area },
                beforeSend: function(){
...
var questioncomments = function(){
    ...
    if(comment_val!=''){
        ...
        $.ajax({
        url:"/employer/allcandidaddcomments",
        type: 'post',
        data: { "candidate_id": candidate_id,"question_id": question_id,"employer_id": employer_id,"comment_val": comment_val},
        beforeSend: function(){
        ...
        success:function(data)
        {
            ...
            if(data.response=='1'){
                $('#'+txtname).val('');
                $('#commentlist'+loopid).load("/employer/allcandqstncommentlist/" + question_id+'/'+ candidate_id);
            }
...
var videocontrols = function(){
    ...
    $( ".video_player_box" ).each(function( index ) {
      ...
      $("#seekslider"+index).on("change", function(e) { // slider moves
      ...
      $("#upload_videotag"+index).on("timeupdate", function(e) { // video tag 
      ...
      if(loaddursecs < 10){ loaddursecs = "0"+loaddursecs; }
      if(loaddurmins < 10){ loaddurmins = "0"+loaddurmins; }
...
KTUtil.onDOMContentLoaded(function () {
  ...
  function vidSeek(seekslider,vid,finish_time,index){
    ...
    var seekto = finish_time * (seekslider.value / 100);
      ...
      seekslider.style.background = `linear-gradient(to right, white 0%, white ${(value-min)/(max-min)*100}%, #b3b4b5 ${(value-min)/(max-min)*100}%, #b3b4b5 100%)`
  
      seekslider.oninput = function() {
        this.style.background = `linear-gradient(to right, white 0%, white ${(this.value-this.min)/(this.max-this.min)*100}%, #b3b4b5 ${(this.value-this.min)/(this.max-this.min)*100}%, #b3b4b5 100%)`
      };
  }
  ...
  function seektimeupdate(vid,seekslider,finish_time,index){
      ...
      const max = seekslider.max;
      const value = seekslider.value;
      seekslider.style.background = `linear-gradient(to right, #0086FF 0%, #0086FF ${(value-min)/(max-min)*100}%, #b3b4b5 ${(value-min)/(max-min)*100}%, #b3b4b5 100%)`
  
      seekslider.oninput = function() {
        this.style.background = `linear-gradient(to right, #0086FF 0%, #0086FF ${(this.value-this.min)/(this.max-this.min)*100}%, #b3b4b5 ${(this.value-this.min)/(this.max-this.min)*100}%, #b3b4b5 100%)`
      };
