...
class ServiceController extends Controller
{
	...
	public function index_working(Request $request) {
		...
		if (isset($_FILES['blob'])) {
			...
			echo json_encode(array('status' => true,'file_name'=>$file_name, 'block' => $_POST['block'],'stepconstid'=>$request->stepconstid));
	...
	public function index111(Request $request) {
		...
		if (isset($_FILES['blob'])) {
			...
			if ($start == 0) {
				...
				for ($count = 0;; $count++) {
					if (filesize($filename) == 0) {
						if ($count > 20) {
							// After 1 second
							echo json_encode(array('status' => false, 'message' => 'File is empty'));
							exit;
						}
						usleep(50000);
						clearstatcache(false, $filename);
			...
			echo json_encode(array('status' => false, 'message' => 'No video chunk received'));
		}
	}
	...
	public function index(Request $request) {
		...
		if ($request->hasFile('blob')) {
			...
			if ($start == 0 && !file_exists($filename)) {
				// Create or overwrite the file if starting from scratch
				$desc = fopen($filename, 'w'); // 'w' will create a new file if it doesn't exist
				fclose($desc);
			}
	
			// Append chunk to the file
			$desc = fopen($filename, 'a');
			$src = fopen($request->file('blob')->getRealPath(), 'r');
			while (!feof($src)) {
				$buf = fread($src, 4194304); // 4M
				fwrite($desc, $buf);
			}
			...
			return response()->json([
				...
				'file_name' => $file_name,
				'filename' => $filename,
				'stepconstid' => $request->input('stepconstid')
			]);
		} else {
			return response()->json([
				'status' => false,
