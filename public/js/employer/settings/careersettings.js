...
/*!********************************************************!*\
  !*** ./Resources/assets/js/settings/careersettings.js ***!
  \********************************************************/
...
var validation;
var Form = document.querySelector('#kt_company_setting_form'); // Class definition
...
var CareerSetting = function () {
  ...
  var logo_image = document.getElementById('logo_image'); // var cropper;

  submitButton = document.querySelector('#kt_career_setting_submit');

  var deletelogoimage = function deletelogoimage() {
  ...
  var imageOpen = function imageOpen() {
    ...
    $("#kt_career_setting_submit").click(function () {
      ...
      validator.validate().then(function (status) {
        if (status == 'Valid') {
          ...
          setTimeout(function () {
            $('#kt_company_setting_form').submit(); // Hide loading indication
  ...
  var bannerimagecrop = function bannerimagecrop() {
    var url = document.location.toString(); // var originalImageURL ;
    ...
    $("#company_photo1").change(function () {
      ...
      $('#image').attr('src', uploadedImageURL); //cropper = new Cropper(image, options);
  ...
  var getcategoryid = function getcategoryid(e) {
    return e.attr('data-catid');
  };

  var usetemplatemdl = function usetemplatemdl() {
  ...
  var insertDescTemplate = function insertDescTemplate() {
    $(".insertDescTemplate").click(function () {
      var inputTags;
      var li_content;
      inputTags = frmusetemplate.getElementsByTagName('input'); // console.log(inputTags);
      ...
      for (var i = 0, length = inputTags.length; i < length; i++) {
        if (inputTags[i].type == 'checkbox' && inputTags[i].checked) {
          ...
          li_content = $('#content_' + checkval).find('li'); // console.log(li_content);
  ...
  var validationurl = function validationurl() {
  ...
  var clipboardcopy = function clipboardcopy() {
    ...
    new ClipboardJS('[data-clipboard1=true]').on('success', function (e) {
      e.clearSelection();
      seturlTooltip('Copied!'); // hideurlTooltip();
    });
    new ClipboardJS('[data-clipboard2=true]').on('success', function (e) {
      e.clearSelection();
      setembedTooltip('Embed Code Copied!'); // hideembedTooltip();
    });
  }; // Public functions
    ...
    success: function success(responseData) {
      if (responseData.response == "success") {
        ...
        $('input:radio[name=lib_radio]').on('change', function () {
          ...
          $(this).closest('.libraryimg_category').find('img').css({
            "margin": "0 5px",
...
function submitinsertform() {
  ...
  console.log(img_src); //$('.fileupload_div').css('display','none');
  ...
  if ($('#photo_id').val() == '1') {
    ...
    $.ajax({
      ...
      // dataType: "json",
      data: {
        "library_filename1": library_filename1,
