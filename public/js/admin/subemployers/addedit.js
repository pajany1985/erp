...
/*!*****************************************************!*\
  !*** ./Resources/assets/js/subemployers/addedit.js ***!
  \*****************************************************/
 // Class definition
...
var KTCreateAccount = function () {
  ...
  var handleForm = function handleForm() {
    ...
    $(form.querySelector('[name="masteremp"]')).on('change', function () {
      // Revalidate the field when an option is chosen
      validations[1].revalidateField('masteremp');
    });
  ...
  var masterempDetails = function masterempDetails() {
    $('#masteremp').on('change', function () {
      $.ajax({
        url: "/admin/subemployers/masterempdetails",
        method: "post",
        data: {
          "emp_id": this.value
        },
        success: function success(data) {
          if (data.success == '1') {
            $('.masteremp_details').html(data.result);
          }
        }
      });
    });
